...
t1=with(filter(pds, era=="1950-2014"), cor.test(mmp, cld, method = "spearman",use="complete"))
t1n=nrow(na.omit(select(filter(pds, era=="1950-2014"),mmp,cld)))

t2=with(filter(pds, era=="1950-2014"), cor.test(mmp ,wcprec, method = "spearman",use="complete"))
t2n=nrow(na.omit(select(filter(pds, era=="1950-2014"),mmp,wcprec)))

td=data.frame(Data=c("CF","WorldClim"),
              "SpearmanRho"=c(t1$estimate,t2$estimate),
              'p-value'=c(t1$p.value,t2$p.value),
              n=c(t1n,t2n))
kable(td)
