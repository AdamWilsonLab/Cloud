___Process monthly precipitation climatologies from the GHCN monthly database___
Adam M. Wilson
...
m2=lm(cld~log(mmp+1),data=pds)
m3=lm(cld~log(mmp+1)+elevation,data=pds)
m4=lm(cld~log(mmp+1)+biomename,data=pds)

#screenreg(list(m1,m2,m3,m4),ci.force=T,single.row=T,omit.coef="biome")
htmlreg(list(m1,m2,m3,m4),ci.force=T,single.row=T,doctype=F,omit.coef="biome")
```