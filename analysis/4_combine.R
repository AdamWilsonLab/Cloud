...
#### old junk below here...
################################################################################
writeRaster(dmean,format="CDF","data/MCD09/MODCF_mean.nc",varname="mean",varunit="%", longname="Cloud Frequency (%)",
            xname="Longitude", yname="Latitude", zname="Month", zunit="days since 2000-01-01")
)

trans_ops=paste(" -co COMPRESS=DEFLATE -a_nodata 65535 -stats -co FORMAT=NC4 -co ZLEVEL=9 ")
...
cat(paste("
          ...
          time:long_name = \"date of observation\"; 
          data:
