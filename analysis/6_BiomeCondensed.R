...
if(!file.exists("data/src/teow/biomes_condensed.shp")){
  library(geosphere)
  library(rgeos)
  ...
  writeOGR(biome,"data/src/teow","biomes_condensed",driver="ESRI Shapefile",overwrite=T)
}

## rasterize the biome dataset
## create a copy with a numeric biome code
biome=readOGR("data/src/teow/","biomes_condensed")
bcode=unique(data.frame(icode=biome$icode,code=biome$code,realm=biome$realm,biome=biome$biome,area=biome$areakm))
write.csv(bcode,"data/out/biomecode_condensed.csv",row.names=F)
...
system(paste("gdal_rasterize -a icode -init 0 -l biomes -ot Byte -te -180 -90 180 90 -tr 0.008333333333333 -0.008333333333333",
              ...
              " data/src/teow/biomes_condensed.shp  data/out/teow/teow_condensed.tif"))
...
foreach(m=bprods)%dopar%{
  ...
  tcloudbiome=paste0("data/out/biomesummaries/teow_condensed_",sub(".tif",".txt",basename(m)))
  ...
  system(paste("pksetmask -i data/out/teow_condensed.tif -m ",m," -ot UInt16 ",
...
write.csv(bs,file="data/out/biomesummary_condensed.csv",row.names=F)

###################################################################
### summary by biome
bs=read.csv(file="data/out/biomesummary_condensed.csv")
