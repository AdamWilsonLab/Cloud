...
\begin{table}[h]
\begin{tabular}{p{.12\textwidth}|p{.3\textwidth}|p{.05\textwidth}|p{.05\textwidth}|p{.1\textwidth}|p{.1\textwidth}|p{.12\textwidth}}
...
%Name & Description & Spatial Domain & Spatial Resolution & Temporal Domain & Temporal Resolution & Reference   \\ \hline
...
<<Sahara,dev='png',fig.cap="Comparison of January cloud frequency over the Southwestern Sahara from A) uncorrected Terra and B) corrected Terra, C) Uncorrected Aqua, and D) Corrected Aqua.  Note the banding in the uncorrected data resulting from variable observation frequency due to orbital artefacts of the MODIS Satellite.",dpi=150,fig.width=8,fig.height=8,fig.align='center',out.width='\textwidth'>>=
...
cldr=(cldml,seas~variable,value="value",fun=fsum,na.rm=T)
