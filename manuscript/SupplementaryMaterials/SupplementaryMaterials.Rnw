...
opts_chunk$set(comment=NA, eval=T,results="hide",echo=F,message=F,fig.width=8, fig.height=8,cache=T,dpi=150,fig.align='center')
...
AVHRR PATMOS-x &  Cloud product derived from NOAA's Advanced Very High Resolution Radiometer (AVHRR)	&Global&	$0.1^o$ ($\approx$11km)&	2010&	Daily& 	\cite{pavolonis2005}\\ \hline
...
<<monthscatter,fig.cap="Mean monthly cloud amount over 1970-2009 from 5388 global stations versus mean 2000-2009 MOD09 cloud frequency by month.  Coefficient of determination is shown in each panel.  Colors represent the number of monthly station observations within each grid cell of the scatterplot.",fig.width=6, fig.height=6>>=
...
t1=cldm %.% group_by(month) %.% summarise(
...
t2=cldm %.% group_by(seas) %.% summarise(
  ...
  R2=lm_summary(mod09,cld,"rs"),
  RMSE=lm_summary(mod09,cld,"rmse"))
colnames(t1)[1]="Time";colnames(t2)[1]="Time"
  
## combine to a single table
vtable=rbind.data.frame(t1,t2)
print(xtable(vtable,digits=2,caption="Summary of validation data by month and season"),"latex")
...
<<seasscatter,fig.width=4, fig.height=4>>=
tr2=c(0,26)
...
texreg(mods,custom.model.names = names(mods),custom.coef.names = c("Intercept", "MODCF"),
        single.row = T,caption="Comparison of validation models for full station record (1970-2009) and MODIS era (2000-2009).")#,caption.placement='top')
@
...
However, due to spatially heterogeneous application of cloud tests (even in the recently reprocessed Collection 6), the MOD35 mask is unsuitable for generating spatially consistent maps of cloud frequencies at 1-km resolution \citep{Wilson2013}.  
